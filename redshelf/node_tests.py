...
class TestNodeMethods(unittest.TestCase):
    ...
    def test_get_bucket(self):
        ...
        node = self.root.get_bucket(self.keys, ['Pearson', '*', '5'])
        self.assertIsNotNone(node)

        node = self.root.get_bucket(self.keys, ['Pearson', '10_day', '6'])
        self.assertIsNotNone(node)

        node = self.root.get_bucket(self.keys, ['Openstax', '*', '*'])
        self.assertIsNotNone(node)

        node = self.root.get_bucket(self.keys, ['*', '*', '*'])
        self.assertIsNotNone(node)
